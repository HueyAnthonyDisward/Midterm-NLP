...
1. Mở terminal tại thư mục `NguyenTrungHieu22110138` (cùng cấp với file `manage.py`).
...
3. Truy cập đường dẫn cục bộ (ví dụ: `http://127.0.0.1:8000`) được hiển thị trên terminal.
...
Nếu có thắc mắc, vui lòng gửi email đến:  
...
1. Open a terminal in the `NguyenTrungHieu22110138` folder (same level as `manage.py`).
...
3. Access the local URL (e.g., `http://127.0.0.1:8000`) displayed in the terminal.
